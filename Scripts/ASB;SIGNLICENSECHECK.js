...
//doStandardChoiceActions(controlString,true,0);
         ...
         // showMessage = true; //show the error
        //  cancel=true;   //stop this from continuing
			
			comment("test");
	   comment(" at bond record P4 " + capId + " CurrentUserID: " + currentUserID);
	   comment(" aa.person.getuser: " + systemUserObj)
	...
	if(AInfo["Illuminated"] == "Yes")
	{
		comment("Illuminated is YES");
		
		if(checkCapForLicensedProfessionalType("Illuminated Signs") == false)
		{
			cancel = true;
			showMessage = true;
			comment("You are not licensed to install Illuminated Signs.");
					}
		else
		{	
			comment("This user is licenced to install this illuminated sign.");
			
		}
	}
	else
		comment("Illuminated is NO");
